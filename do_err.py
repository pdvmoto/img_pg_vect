...
pp    ( ' ' )
pp    ( ' processing with erro-catching ...' )
pp    ( ' .' )
...
try:
  ...
  for row in cursor.execute ( sql_for_qry ):
    # pp    ( ' .. inside for  ...' )
    rowcnt = rowcnt + 1 
    pp   ( row , ' rownum: ', cursor.rowcount )
  # pp    ( ' .. end of try , forloop done ...' )
except Exception as inst:
  pp    ( ' .. catchig exception:' )
  pp    ( '   .. type: ', type(inst))    # the exception type
  # pp  ( '   .. args: ', inst.args)     # arguments stored in .args
  pp    ( '   .. inst: ', inst )         # __str__ allows args to be printed directly,
                       # but may be overridden in exception subclasses
  #     f_inspect_obj ( 'ist', inst ) 

  pp    ( '   .. SQL : ', cursor.statement )
  pp    ( ' .. end of exception, error is done ...' )
